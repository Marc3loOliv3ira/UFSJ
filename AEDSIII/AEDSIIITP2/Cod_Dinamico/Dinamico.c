...
int main(int argc, char *argv[])
{
	...
	int eh_digrafo = 0;
	int *auxiliar;
	char auxiliar1[256]; //vari?vel auxiliar para salvar o nome do arquivo que ser? aberto.
	char auxiliar2[256];
	...
	if (habilit + habilit1 == 2)
	{
		...
		for(k = 0; k < instancias; k++)
		{
			...
			//printf("%d %.0f %d %d\n",Vertices,Distancia,Peso_Nave,Caminhos);//Num de Vertices,Grau Maximo,Se é ou nao Ponderado
			vetpeso = malloc(Vertices * sizeof(int));
			vethab = malloc(Vertices * sizeof(int));
			auxiliar = malloc(Vertices * sizeof(int));
